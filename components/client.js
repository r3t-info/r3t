...
export default function Client({ clientData }) {
  return (
    <div
      ...
      className="bg-light border-solid border-3 border-accent rounded-lg p-2 md:px-6 md:py-4 lg:px-4 flex flex-col lg:w-1/3 lg:mx-6 xl:mx-8 lg:self-start lg:mb-6"
    >
      <div id={`client-header-${clientData.id}`} className="w-full flex flex-row flex-nowrap pb-4">
        <div className="flex-shrink-0">
          <img src={clientData.img} alt={clientData.name} className="h-auto w-20 sm:w-24 xl:w-32 rounded-lg" />
        </div>
